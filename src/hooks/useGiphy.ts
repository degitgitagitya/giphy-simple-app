...
interface UseGiphyParams {
  q: string;
}

const useGiphy = ({ q }: UseGiphyParams) => {
  const { data, error, isLoading } = useQuery<GetGiphyResponse, string>(
    ['getGiphy', q],
    async () => {
      const response = await fetch(
        `${GIPHY_API_URL}/${GIPHY_API_VERSION}/gifs/search?api_key=${
          import.meta.env.VITE_API_KEY
        }&q=${q}&limit=9&offset=0&rating=g&lang=en`
      );

      const result = (await response.json()) as GetGiphyResponse;

      if (result.meta.status === 200) return result;

      throw Error(result.meta.msg);
    },
    {
      enabled: q !== '',
    }
